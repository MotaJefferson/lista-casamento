                    ...
                    config.attractions.map((attr, idx) => (
                        <div key={idx} className="relative group pl-4 border-l-2 border-primary/20 hover:border-primary transition-colors py-2">
                            <div className="flex justify-between items-baseline mb-1">
                                <span className="font-bold text-foreground text-lg">{attr.title}</span>
                                <span className="text-xs font-bold text-primary bg-primary/5 px-2 py-0.5 rounded">{attr.time}</span>
                            </div>
                            {attr.description && <p className="text-sm text-muted-foreground line-clamp-2 leading-relaxed">{attr.description}</p>}
                        </div>
                    ))
                ) : (
